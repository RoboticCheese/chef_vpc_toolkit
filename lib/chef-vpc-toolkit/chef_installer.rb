...
echo "Starting notification server on: $HOSTNAME"
EOF_BASH
EOF_GATEWAY
}
puts data
...
ssh -o "StrictHostKeyChecking no" root@#{options['ssh_gateway_ip']} bash <<-"EOF_GATEWAY"
ssh #{options['chef_server_name']} bash <<-"EOF_BASH"
echo "Started notification server"
...
def self.start_chef_clients(options, restart=false)
json=JSON.parse(IO.read(options['chef_json_file']))
json.each_pair do |hostname, json_hash|
    start_chef_client(options, hostname, restart)
end
end

def self.start_chef_client(options, client_name, restart=false)
